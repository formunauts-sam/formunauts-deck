...
/* ============================================================
   FORMUNAUTS brand wrapper — vendored addition (NOT upstream).
   ------------------------------------------------------------
   Everything above this line is paulmillr's `qr` (npm: qr,
   github paulmillr/qr), an ESM-native, zero-dependency, CDN-free
   QR encoder, vendored verbatim (dual Apache-2.0 / MIT; see
   vendor/qr.LICENSE) exactly like dist/reveal.mjs. It runs with
   NO network call and NO external import.

   The upstream encodeQR(text,'svg') emits a monochrome
   black-on-transparent path with a compact 2-module quiet zone
   and no colour control. The Engage Join panel needs brand-blue
   modules on white with a GENEROUS quiet zone that stays crisp on
   a projector, so we build the SVG ourselves from the raw module
   matrix (encodeQR(text,'raw')) and hand-author the markup with
   brand tokens. One rect per dark module keeps the string small
   and every edge pixel-snapped (no anti-alias smear on a beamer).

   qrSvg(text, opts) -> string   (an <svg>...</svg> STRING)
     opts.ecc     'low'|'medium'|'quartile'|'high'  (default 'medium';
                  §4.7 asks for medium error correction)
     opts.quiet   quiet-zone width in modules (default 4, the
                  ISO/IEC 18004 §9.1 minimum; generous by design)
     opts.size    target pixel edge of the whole SVG incl. quiet
                  zone (default 512; the viewBox is in module units
                  so it also scales crisply to any CSS size)
     opts.dark    module colour  (default brand blue #0074C8)
     opts.light   background colour (default white #FFFFFF)
     opts.title   accessible <title> (default "Join QR code")
   Throws only for an empty payload; capacity overflow surfaces the
   upstream encoder's error so the caller can shorten the URL.
============================================================ */

/* Brand defaults kept in one place — mirrors the palette in
   template/brand.css (--fmnts-primary, white surface). We hardcode
   the hex here (not var(--...)) because a QR module fill must be a
   concrete colour: the SVG is often rendered standalone (a fresh
   projector window, a saved image) with no brand stylesheet in scope. */
const QR_BLUE = "#0074C8"; // --fmnts-primary
const QR_WHITE = "#FFFFFF"; // --bg-surface / white
const QR_DEFAULT_QUIET = 4; // modules; ISO/IEC 18004 §9.1 minimum
const QR_DEFAULT_SIZE = 512; // px edge of the rendered SVG
const QR_DEFAULT_ECC = "medium"; // §4.7: medium error correction

/* Escape a string for safe inclusion in SVG/XML text or an attribute.
   The Join URL and title are the only dynamic strings that reach markup;
   both are presenter-authored, but we escape anyway so a stray & or <
   never produces invalid XML. */
...
/* Build a crisp, brand-blue-on-white QR as an SVG STRING.
   Byte mode is auto-selected upstream from the payload (a URL is byte
   mode); we never force numeric/alphanumeric. The smallest version that
   fits is auto-picked by encodeQR when no version is passed. */
export function qrSvg(text, opts = {}) {
  const payload = typeof text === "string" ? text : String(text || "");
  if (!payload) throw new Error("qrSvg: empty payload");

  const ecc = opts.ecc || QR_DEFAULT_ECC;
  const quiet =
    Number.isSafeInteger(opts.quiet) && opts.quiet >= 1
      ? opts.quiet
      : QR_DEFAULT_QUIET;
  const px =
    Number.isSafeInteger(opts.size) && opts.size > 0
      ? opts.size
      : QR_DEFAULT_SIZE;
  const dark = opts.dark || QR_BLUE;
  const light = opts.light || QR_WHITE;
  const title = opts.title || "Join QR code";

  // Let the upstream encoder add the quiet zone (border, in modules): its
  // toRaw() returns the symbol already padded with `quiet` false-cells on
  // every side, so the matrix IS the full grid and its dark cells are the
  // real modules. (encodeQR rejects border <= 0, hence the quiet >= 1 clamp
  // above; the ISO minimum of 4 is our default anyway.)
  const matrix = encodeQR(payload, "raw", { ecc, border: quiet });
  const dim = matrix.length; // square grid incl. quiet zone, in modules

  // viewBox is in MODULE units (0 0 dim dim) so the SVG scales to any pixel
  // size without re-computing paths; width/height give a sensible default.
  // One <rect> per dark module. Coordinates are whole integers so every
  // module edge snaps to the pixel grid on a beamer (no anti-alias smear).
  const rects = [];
  for (let y = 0; y < dim; y++) {
    const row = matrix[y];
    for (let x = 0; x < dim; x++) {
      if (!row[x]) continue; // light / quiet module — leave the white ground
      rects.push(`<rect x="${x}" y="${y}" width="1" height="1"/>`);
    }
  }

  // shape-rendering:crispEdges kills anti-alias fuzz between adjacent modules;
  // the modules share one <g fill> so the string stays compact.
  return (
    `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${dim} ${dim}" ` +
    `width="${px}" height="${px}" role="img" ` +
    `aria-label="${xmlEscape(title)}" ` +
